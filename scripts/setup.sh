...

{
  ...
  echo "# DNS via WireGuard server VPN IP — avoids routing loop when endpoint is a LAN IP"
  echo "WG_DEFAULT_DNS='10.8.0.1'"
  echo "# AllowedIPs: only VPN subnet. Adding the server LAN subnet causes a routing loop"
  echo "# when the WireGuard endpoint is the server's LAN IP."
  echo "WG_ALLOWED_IPS='10.8.0.0/24'"
